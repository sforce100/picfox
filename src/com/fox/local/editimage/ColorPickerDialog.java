	...
	private static class ColorPickerView extends View {
		...
		public boolean onTouchEvent(MotionEvent event) {
			...
			case MotionEvent.ACTION_UP:
				if (mTrackingCenter) {
					if (inCenter) {
						mListener.colorChanged(mCenterPaint.getColor(),size);
					}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		OnColorChangedListener l = new OnColorChangedListener() {
			public void colorChanged(int color,int size) {
				// 调用观察者的接口
				mListener.colorChanged(color,size);
				dismiss();
			}
		};
		LinearLayout layout = new LinearLayout(context);
		layout.setOrientation(LinearLayout.VERTICAL);
		layout.addView(penSize,new LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.WRAP_CONTENT));
